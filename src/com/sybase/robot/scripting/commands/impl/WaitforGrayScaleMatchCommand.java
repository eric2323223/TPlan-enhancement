...
public class WaitforGrayScaleMatchCommand extends AbstractCommandHandler implements AdvancedCommandHandler {
    ...
    public String[] getCommandNames() {
        return new String[]{"waitforgrayscale"};
    }
        ...
        WaitForListener(Map params, ScriptingContext context) {
            ...
            if (event.equalsIgnoreCase(EVENT_MATCH) || event.equalsIgnoreCase(EVENT_MISMATCH)) {
                ...
                if (!comparisonModule.isSecondImageRequired()) {
                    ...
                    modules.add(comparisonModule);
                    comparisonModule.setBaseImage(ImageUtil.convert(ImageUtil.imageToBufferedImage(templateImages.get(0)), BufferedImage.TYPE_BYTE_GRAY));
                    
//                    detector.writeEdgeImageToFile(new File("D:\\workspace_android2\\robot\\template.png"));
//                    comparisonModule.setBaseImage(templateImages.get(0));
                    ...
                    for (int i = 1; i < templateImages.size(); i++) {
                        comparisonModule = ImageComparisonModuleFactory.getInstance().getModule(method);
                        modules.add(comparisonModule);
                        comparisonModule.setBaseImage(ImageUtil.convert(ImageUtil.imageToBufferedImage(templateImages.get(i)), BufferedImage.TYPE_BYTE_GRAY));
                        if (comparisonModule instanceof ExtendedParamsObject && modParams != null) {
                            ((ExtendedParamsObject) comparisonModule).setParameters(modParams);
                        }
                    }
        ...
        public void run() {
            ...
            ImageComparisonModule comparisonModule = new SearchImageComparisonModule();
            int i = 0;
...
//                        break;
                    ...
                    try {
                        ...
                        rate = 100 * comparisonModule.compareToBaseImage(ImageUtil.convert(ImageUtil.imageToBufferedImage(client.getImage()), BufferedImage.TYPE_BYTE_GRAY), cmpArea, methodParams, context, passrate / 100f);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                        rate = 0;
                    }
